...
public class BlockNylium extends Block implements IBlockFragilePlantElement {
    ...
    public void performBonemeal(WorldServer worldserver, Random random, BlockPosition blockposition, IBlockData iblockdata) {
        ...
        if (iblockdata1.is(Blocks.CRIMSON_NYLIUM)) {
            ((WorldGenFeatureConfigured) NetherFeatures.CRIMSON_FOREST_VEGETATION_BONEMEAL.value()).place(worldserver, chunkgenerator, random, blockposition1);
        } else if (iblockdata1.is(Blocks.WARPED_NYLIUM)) {
            ((WorldGenFeatureConfigured) NetherFeatures.WARPED_FOREST_VEGETATION_BONEMEAL.value()).place(worldserver, chunkgenerator, random, blockposition1);
            ((WorldGenFeatureConfigured) NetherFeatures.NETHER_SPROUTS_BONEMEAL.value()).place(worldserver, chunkgenerator, random, blockposition1);
            if (random.nextInt(8) == 0) {
                ((WorldGenFeatureConfigured) NetherFeatures.TWISTING_VINES_BONEMEAL.value()).place(worldserver, chunkgenerator, random, blockposition1);
            }
        }
